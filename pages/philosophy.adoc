...
Mutiny can be used in any Java application exhibiting asynchrony. From reactive microservices, data streaming, event processing to API gateway, and network utility, Mutiny is a great fit.

=== Why is asynchronous important? 
We are living in a distributed worlds. 
...
But, distributed systems are challenging.

.Distributed systems are asynchronous
image::/assets/images/distributed-system-blueprint.png[role="left"]

Their nature is inherently asynchronous and wild. 
...
Building distributed applications correctly is a considerable challenge. 
It requires reassessing almost everything we know about development. 

Typically, most applications use a synchronous development model. 
...
When the request processing needs to interact over the network, it uses that _worker_ thread, which blocks the thread until the response's reception. 
...
Asynchronous is a shift. 
But, Mutiny helps you to take the plunge.

=== What's Reactive Programming

As said above, Mutiny is a reactive programming library.
...
With reactive programming, you observe streams implement side-effects when _something_ flows in the stream:

.Reactive programming is about observing streams
image::/assets/images/streams.png[role="center"]
...
As said above, asynchronous is hard to grasp for most developers. 
Thus, the API must not require advanced knowledge or add cognitive overload. It should help you design your logic and be understandable in 6 months.
...
Also, `Uni` does not need the complete ceremony presented above as the _request_ does not make sense.
The `subscribe` event expresses the interest and triggers the computation, no need for an additional _request_.
Also, `Uni` can handle items having a `null` items (and has specific methods to handle this case).
